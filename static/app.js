...
clearBtn?.addEventListener("click", () => {
  chatEl.innerHTML = "";
  addMessage("New chat started. How can I help?", false);
});

// reset session (register ONCE, not inside submit)
resetBtn?.addEventListener("click", async () => {
  ...
  chatEl.innerHTML = "";
  addMessage("Session reset. How can I help?", false);
  inputEl?.focus();
});
...
// optional greeting (only if chat is empty on load)
if (chatEl && chatEl.children.length === 0) {
  addMessage("Hey there! I’m ready. Ask me anything.", false);
}
...
formEl?.addEventListener("submit", async (e) => {
  ...
  try {
    const data = await sendToServer(msg);
    ...
    const botEl = addMessage("", false);
    const botContent = botEl?.querySelector?.(".content") || botEl;

    // If template structure differs, fall back to msg element itself
    if (botContent && botContent !== botEl) {
      await typeInto(botContent, replyText);
    } else {
      await typeInto(botEl, replyText);
    }
  } catch (err) {
    typingEl?.remove();
