...
# Flask setup (keeps your capitalized Templates folder + static)
...
def ensure_model_loaded():
    ...
    with model_lock:
        ...
        mdl = AutoModelForCausalLM.from_pretrained(
            ...
            device_map="auto"            # CPU unless a GPU is present
        )
        mdl.eval()
        tokenizer = tok
        model = mdl
...
def safe_eval_expr(expr: str):
    ...
    val = _eval_node(tree)
    return int(val) if isinstance(val, float) and hasattr(val, "is_integer") and val.is_integer() else val
...
if __name__ == "__main__":
    # Render binds you to PORT and 0.0.0.0
